(function () {
  ...
  async function compareSourceAndDestinationTotals(extractionMeta) {
    ...
    const totalField = findDestinationNetTotalField();
    if (!totalField) {
      await InvoiceLogger.logEvent("warn", "destination-content", "destination-total-missing", "");
      return null;
    }
  ...
  function findDestinationNetTotalField() {
    return (
  ...
  function getDestinationRowCells(row) {
    return Array.from(row?.cells || []);
  }

  function getDestinationQuantity(row) {
    const cells = getDestinationRowCells(row);
